"""Benchmark runner for PSO (base) on TMLAP instances.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Ejecuta benchmark PSO base sobre instancias TMLAP definidas en txt "
...
def _run_pso(
    ...
    span = float(ub - lb)
    v_max = max(0.0, float(v_max_frac) * span)
    ...
    fitness = np.array([objective(ind) for ind in positions], dtype=float)
    ...
    for iteration in range(max_iter):
        ...
        candidate_idx = int(np.argmin(pbest_fitness))
        candidate_fit = float(pbest_fitness[candidate_idx])
        if candidate_fit < gbest_fitness:
            gbest_fitness = candidate_fit
            gbest_position = pbest_positions[candidate_idx].copy()

        convergence[iteration] = gbest_fitness
...
def main() -> None:
    ...
    run_name = f"tmlap_pso_r{args.runs}_{stamp}"
    if args.tag.strip():
    ...
    with (
        ...
        # Header-only file to preserve output compatibility across algorithms.
        ...
        for instance_path in instances:
            ...
            for run_idx in range(1, args.runs + 1):
                ...
                t0 = time.time()
                best_fitness, best_position, convergence_curve = _run_pso(
                ...
                _ = problem.repair_from_latent(best_position)
