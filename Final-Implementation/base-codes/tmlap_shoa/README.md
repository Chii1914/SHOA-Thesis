# TMLAP - SHOA (Base Implementation)

Implementacion base autonoma de SHOA para resolver TMLAP.

## 1) Objetivo

Ejecutar benchmark TMLAP con SHOA puro usando nucleo local (sin depender de python-code externo).

## 2) Estructura local

- `run_benchmark.py`: entrypoint local (smoke/full/profiles).
- `benchmarks/run_tmlap_shoa_puro_benchmark.py`: benchmark principal.
- `benchmarks/run_tmlap_shoa_puro_profiles_matrix.py`: matriz de perfiles.
- `benchmarks/shoa_puro_tmlap_profiles_config.json`: perfiles soft/medium/hard.
- `python-code/SHO.py`, `initialization.py`, `levy.py`: nucleo SHOA local.
...
Cada corrida crea carpeta en `results/benchmark_logs/tmlap_shoa_puro_*`.
