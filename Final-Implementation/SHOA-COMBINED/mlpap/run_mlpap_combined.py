"""SHOA-COMBINED runner for MLPAP instances (SHOA + LIME + stagnation detection)."""
...
LIME_FIELDNAMES = [
    "run_id", "timestamp", "function_name", "dimension",
    "diagnosis_id", "iteration", "target_type", "feature",
    "mean_weight", "abs_mean_weight",
    "trigger_source", "trigger_event", "trigger_iteration",
]

GLOBAL_FIELDNAMES = [
    "run_id", "function_name", "dimension",
    "target_type", "feature", "window", "window_size", "If", "Sf",
]


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run SHOA-COMBINED on MLPAP instances")
    ...
    parser.add_argument("--lime-enabled", type=int, default=1, choices=[0, 1])
    parser.add_argument("--lime-min-samples", type=int, default=400)
    parser.add_argument("--stagnation-lime-selection-mode", type=str,
                        default="medoid", choices=["selected_agents", "medoid"])
    parser.add_argument("--lime-selection-mode", type=str, default=None,
...
def main() -> None:
    ...
    restart_cooldown_fes = max(1, int(round(float(cooldown_ratio) * base_max_fes)))
    ...
    write_csv(run_dir / "lime_contributions.csv", lime_rows_all)
    write_csv(run_dir / "global_feature_explanations.csv", global_rows_all)
